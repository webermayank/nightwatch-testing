...
module.exports = class StaticAssert {
  ...
  createStaticAssertion(commandName, abortOnFailure, apiToReturn) {
    class Assertion extends EventEmitter {
      constructor({ negate, args }) {
        ...
        this.message =
          (isLastArgString &&
            (args.length > 2 || Utils.isBoolean(args[0])) &&
            lastArgument) ||
      ...
      getMessage(propName) {
        ...
        const operator = this.passed
          ? StaticAssert.assertOperators[propName][0]
          : StaticAssert.assertOperators[propName][1];
      ...
      assert(propName) {
        const customMessage = "Testing the value to be";
        try {
          assertModule[propName].apply(null, this.args);
          this.passed = !this.negate;
          this.message = `${this.negate ? "Failed" : "Passed"} [${propName}]:${
            this.args[1] || customMessage + " truthy"
          }: (${this.getMessage(propName)})`;
        } catch (ex) {
          this.passed = !!this.negate;
          ...
          if (!this.passed && ex instanceof Error && propName === "fail") {
            ...
            this.message = `${this.negate ? "Passed" : "Failed"} [${
              this.negate ? "not." : ""
            }${propName}]: ${
              this.args[1] || customMessage + " not truthy"
            }: (${this.getMessage(propName)})`;
          }
  ...
  loadStaticAssertions(parent = null) {
    Object.keys(assertModule).forEach((prop) => {
      ...
      if (!parent) {
        this.nightwatchInstance.setNamespacedApiMethod(
          prop,
          "assert",
          function (prop) {
            return this.createStaticAssertion(
              prop,
              true,
              namespacedApiToReturn("assert")
            );
          }.bind(this)(prop)
        );
      }
  ...
  loadStaticExpect(parent = null) {
    try {
      this.nightwatchInstance.setApiMethod("expect", parent, (...args) => {
        let obj = args[0];
        const isElement =
          Element.isElementObject(obj) ||
          (Utils.isObject(obj) && obj["@nightwatch_element"]);
