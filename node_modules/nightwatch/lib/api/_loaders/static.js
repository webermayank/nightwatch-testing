  ...
  createStaticAssertion(commandName, abortOnFailure, apiToReturn) {
    class Assertion extends EventEmitter {
      ...
      getMessage(propName) {
        ...
        let message = `Testing the value to ${this.negate ? 'not ' : ''}be `;

  if (propName === 'ok') {
    message += this.negate ? 'truthy' : 'not truthy';
  } else {
    message += `${operator}`;
  }

  return message + `: expected ${util.inspect(this.expected)}, but got ${util.inspect(this.actual)}`;
      }
